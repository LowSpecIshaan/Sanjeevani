...
def chat():
    ...
    # System prompt + user message (single string!)
    prompt_text = (
        "You are a healthcare assistant for migrant workers in Kerala. "
        "Generate a concise preliminary diagnosis based on the user query. "
    ...
    try:
        ...
        if not reply and "choices" in result and len(result["choices"]) > 0:
            reply = result["choices"][0].get("text") or result["choices"][0].get("message")

        if not reply:
            reply = "Sorry, no response from the model."
...
@app.route('/schemes')
def schemes():
    return render_template('shemes.html')
